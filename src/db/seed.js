...
//
// Two things to know:
//   • Records now carry client-generated UUIDs, so rows must be minted
//     through bulkSeed() below rather than relying on auto-increment keys.
//     (The old code read `empIds[0]` from bulkAdd's return value, which is
//     the LAST key, not an array — those foreign keys were already broken.)
//   • Seeded rows are written as 'synced' and WITHOUT a farmId so the sync
//     engine never pushes fake cows into a real farm's cloud database.
...
/**
 * Insert demo rows with minted ids and return those ids in order, so
 * `const empIds = await bulkSeed(db.employees, [...])` then `empIds[0]`
 * resolves to a real foreign key.
 */
async function bulkSeed(table, rows) {
  const withIds = rows.map((r) => ({
    ...r,
    id: r.id || newId(),
    // No farmId: keeps demo data out of the push queue.
    syncStatus: 'synced',
    deletedAt: null,
  }));
  await table.bulkPut(withIds);
...
export async function seedDemoData() {
  ...
  await db.transaction('rw', [
    ...
    await bulkSeed(db.attendance, attEntries);
