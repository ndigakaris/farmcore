...
function EmployeeForm({ initial={}, onClose }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2"><label className="form-label">Full Name<span className="text-red-500">*</span></label>
        <input className="form-input" value={form.name} onChange={e=>f('name',e.target.value)}/></div>
      ...
      <div><label className="form-label">Phone</label>
        <input className="form-input" value={form.phone} onChange={e=>f('phone',e.target.value)} placeholder="0712 345 678"/></div>
      <div><label className="form-label">National ID</label>
        <input className="form-input" value={form.nationalId} onChange={e=>f('nationalId',e.target.value)}/></div>
      <div><label className="form-label">Hire Date</label>
        <input className="form-input" type="date" value={form.hireDate} onChange={e=>f('hireDate',e.target.value)}/></div>
      <div><label className="form-label">Monthly Salary (KES)</label>
        <input className="form-input" type="number" value={form.salary} onChange={e=>f('salary',e.target.value)}/></div>
      <div><label className="form-label">Resignation Date <span className="text-gray-400 text-xs">(if resigned)</span></label>
        ...
        {form.resignationDate && <p className="text-xs text-amber-600 mt-1">⚠️ Will be marked as resigned</p>}</div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1">
        {active.length===0 && <p className="text-sm text-gray-400 text-center py-6">No active employees found. Add employees first.</p>}
        {active.map(e=>(
          <div key={e.id} className="flex items-center gap-3 bg-[#F5F0E8] rounded-xl px-3 py-2.5">
            ...
            <div className="flex gap-1">
              {[['present','✅'],['absent','❌'],['leave','🏖️']].map(([s,em])=>(
                <button key={s} onClick={()=>setStatus(e.id,s)}
                  className={`px-2 py-1 rounded-lg text-xs font-medium border transition-all ${(records[e.id]||'present')===s?'bg-[#2D5016] text-white border-[#2D5016]':'bg-white border-[#e8e0d0] text-gray-600'}`}>
                  {em} {s.charAt(0).toUpperCase()+s.slice(1)}
                </button>
              ))}
            </div>
            {(records[e.id]||'present')!=='present' && (
              <input className="form-input w-28 py-1 text-xs" value={notes[e.id]||''}
                onChange={ev=>setNote(e.id,ev.target.value)} placeholder="Reason…"/>
            )}
          </div>
        ))}
      </div>
...
function PayrollForm({ onClose }) {
  ...
  const NSSF=0.06; const NHIF=0.015;

  const rows = active.map(e=>{
    const basic = parseFloat(overrides[e.id]?.basic ?? e.salary)||0;
    const nssf  = parseFloat(overrides[e.id]?.nssf  ?? (basic*NSSF).toFixed(0))||0;
    const nhif  = parseFloat(overrides[e.id]?.nhif  ?? (basic*NHIF).toFixed(0))||0;
    return { e, basic, nssf, nhif, net: basic-nssf-nhif };
  });
  ...
  return (
    <div className="space-y-4">
      <div><label className="form-label">Payroll Month</label>
        <input className="form-input" type="month" value={month} onChange={e=>setMonth(e.target.value)}/></div>
      {active.length===0
        ? <p className="text-sm text-gray-400 text-center py-6">No active employees found.</p>
        : (<>
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead><tr className="bg-[#eef5dd]">
              ...
              <tbody>
                {rows.map(({e,basic,nssf,nhif,net})=>(
                  <tr key={e.id} className="border-b border-[#F5F0E8]">
                    ...
                    <td className="table-td text-right"><input className="form-input py-0.5 text-right w-16 text-xs" value={overrides[e.id]?.nssf??((basic*NSSF).toFixed(0))} onChange={ev=>setOv(e.id,'nssf',ev.target.value)}/></td>
                    <td className="table-td text-right"><input className="form-input py-0.5 text-right w-16 text-xs" value={overrides[e.id]?.nhif??((basic*NHIF).toFixed(0))} onChange={ev=>setOv(e.id,'nhif',ev.target.value)}/></td>
                    <td className="table-td text-right font-bold text-green-700">KES {net.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot><tr className="bg-[#eef5dd] font-bold text-sm">
                <td className="table-td" colSpan={4}>Total Net Payroll</td>
                <td className="table-td text-right text-green-800">KES {totalNet.toLocaleString()}</td>
              </tr></tfoot>
            </table>
          </div>
          <p className="text-xs text-green-700">💡 Payroll auto-recorded as Labour expense in Finance</p>
        </>)}
      <div className="flex justify-end gap-3 pt-2 border-t border-[#e8e0d0]">
        <button className="btn btn-secondary" onClick={onClose}>Cancel</button>
        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving?'Saving…':'Save Payroll'}</button>
      </div>
    </div>
  );
}
...
export default function Employees() {
  ...
  const resigned    = useMemo(()=>(employees||[]).filter(e=>e.status==='resigned').length,[employees]);
  const totalSalary = useMemo(()=>(employees||[]).filter(e=>(e.status||'active')==='active').reduce((s,e)=>s+(e.salary||0),0),[employees]);
  ...
  const workerCols = [
    ...
    { key:'resignationDate', label:'Resignation', render:v=>v?<span className="text-red-600 text-xs">{formatDate(v)}</span>:<span className="text-gray-400 text-xs">—</span> },
  ...
  return (
    <div className="page-content">
      <PageHeader title="Employee Management"
        actions={
          <div className="flex gap-2 flex-wrap">
            ...
            <button className="btn btn-secondary" onClick={()=>setShowPayroll(true)}><DollarSign size={14}/>Run Payroll</button>
      ...
      <StatGrid cols={5}>
        ...
        <KPICard label="Resigned"        value={resigned}          icon="🚪" color={resigned>0?'#d97706':undefined}/>
        ...
        <KPICard label="Monthly Payroll" value={formatCurrency(totalSalary)} icon="💰"/>
      </StatGrid>
      ...
      <div className="card">
        {activeTab==='workers'    && <DataTable columns={workerCols}  rows={filtered}       emptyText="No employees yet. Click 'Add Employee' above."/>}
        {activeTab==='attendance' && (
          attendance && attendance.length > 0
            ? <DataTable columns={attendCols} rows={attendance} emptyText=""/>
            : <div className="text-center py-12">
                <p className="text-4xl mb-3">📋</p>
                <p className="text-sm font-medium text-gray-600 mb-1">No attendance recorded today</p>
                <p className="text-xs text-gray-400 mb-4">Click the Record Attendance button above to mark today's attendance</p>
                <button className="btn btn-primary" onClick={()=>setShowAttend(true)}><Check size={14}/>Record Attendance Now</button>
              </div>
        )}
        {activeTab==='payroll' && (
          payroll && payroll.length > 0
            ? <DataTable columns={payrollCols} rows={payroll} emptyText=""/>
            : <div className="text-center py-12">
                <p className="text-4xl mb-3">💰</p>
                <p className="text-sm font-medium text-gray-600 mb-1">No payroll for {today.slice(0,7)}</p>
                <p className="text-xs text-gray-400 mb-4">Click Run Payroll to process this month's salaries</p>
                <button className="btn btn-primary" onClick={()=>setShowPayroll(true)}><DollarSign size={14}/>Run Payroll Now</button>
              </div>
        )}
      </div>
