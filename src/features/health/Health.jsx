...
import { Plus, AlertTriangle, ShieldCheck, FileDown, Search } from 'lucide-react';

function TreatmentForm({ onClose }) {
  const [form, setForm] = useState({ animalId:'', date:todayStr(), diagnosis:'', customDiagnosis:'', symptoms:'', vet:'Dr. Kamau', treatment:'', cost:'', withdrawal:0, notes:'' });
  ...
  return (
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 flex items-start gap-2">
        ...
        <p className="text-xs text-amber-700">All treatment records are <strong>non-deletable</strong> per antibiotic compliance regulations. Costs are automatically recorded as expenses in Finance.</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
          ...
          <select className="form-input" value={form.animalId} onChange={e=>f('animalId',e.target.value)}>
            ...
            {filteredAnimals.map(a=>(
              <option key={a.id} value={a.id}>
                {SPECIES[a.species]?.emoji} {a.name} {a.tag}{a.animalCode?` (${a.animalCode})`:''}
              </option>
            ))}
          </select>
        </div>
        ...
        {/* Diagnosis with Other option */}
        <div className="col-span-2">
          <label className="form-label">Diagnosis<span className="text-red-500">*</span></label>
          <select className="form-input" value={form.diagnosis} onChange={e=>f('diagnosis',e.target.value)}>
            <option value="">Select diagnosis…</option>
            {diagnosesList.map(d=><option key={d} value={d}>{d}</option>)}
            <option value="Other">Other (specify below)</option>
          </select>
        </div>
        {form.diagnosis === 'Other' && (
          <div className="col-span-2">
            <label className="form-label">Specify Diagnosis<span className="text-red-500">*</span></label>
            <input className="form-input" value={form.customDiagnosis} onChange={e=>f('customDiagnosis',e.target.value)} placeholder="Enter custom diagnosis…"/>
          </div>
        )}
        <div className="col-span-2">
        ...
        <div>
          ...
          {withdrawalEnd && <p className="text-xs text-red-600 mt-1">⚠️ Milk lock until <strong>{withdrawalEnd}</strong></p>}
        </div>
        <div className="col-span-2">
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2">
        ...
        <div className="relative mb-2">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/>
          <input className="form-input pl-8" placeholder="Search animal…" value={animalSearch} onChange={e=>setAnimalSearch(e.target.value)}/>
        </div>
        <select className="form-input" value={form.animalId} onChange={e=>f('animalId',e.target.value)}>
          <option value="">Select…</option>
          {filteredAnimals.map(a=><option key={a.id} value={a.id}>{SPECIES[a.species]?.emoji} {a.name} {a.tag}</option>)}
        </select>
      </div>
...
export default function Health() {
  ...
  const upcomingVax   = useMemo(() => (vaccinations||[]).filter(v=>{ if(!v.nextDue) return false; const d=Math.floor((new Date(v.nextDue)-new Date())/86400000); return d>=0&&d<=30; }).length, [vaccinations]);

  const treatCols = [
    { key:'animalId', label:'Animal', render:(_,row)=>{ const a=animals?.find(x=>x.id===row.animalId); return a?<div><p className="font-medium">{a.name} {a.tag}</p><p className="text-xs text-gray-400">{a.animalCode}</p></div>:'—'; }},
  ...
  return (
    <div className="page-content">
      <PageHeader title="Health & Veterinary"
        actions={
      ...
      {lockedAnimals.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-5">
          ...
          {lockedAnimals.map(a=>(
            <div key={a.id} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-red-100 mb-1">
              <div><span className="font-medium text-sm">{a.name} {a.tag}</span><span className="text-xs text-gray-500 ml-2">{a.lockReason}</span></div>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="btn btn-secondary text-xs"><FileDown size={13}/> Export Audit Log</button>
      </div>
      <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 mb-4 flex items-center gap-2">
        <ShieldCheck size={16} className="text-amber-600"/>
        <p className="text-xs text-amber-700"><strong>Antibiotic Compliance:</strong> All records are non-deletable. Treatment costs auto-appear in Finance as Veterinary expenses.</p>
      </div>
      <div className="card">
        ...
        {activeTab==='mortality'    && <DataTable columns={[
          { key:'animalId', label:'Animal', render:(_,row)=>{ const a=animals?.find(x=>x.id===row.animalId); return a?`${a.name} ${a.tag}`:'—'; }},
          { key:'date',  label:'Date',     render:v=>formatDate(v) },
