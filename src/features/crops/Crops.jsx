...
function PlotForm({ onClose }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2"><label className="form-label">Plot Name<span className="text-red-500">*</span></label>
        <input className="form-input" value={form.name} onChange={e=>f('name',e.target.value)} placeholder="e.g. North Field"/></div>
      <div><label className="form-label">Size</label>
        <input className="form-input" type="number" value={form.size} onChange={e=>f('size',e.target.value)}/></div>
      ...
      <div><label className="form-label">Soil Type</label>
        <input className="form-input" value={form.soilType} onChange={e=>f('soilType',e.target.value)} placeholder="e.g. Red Loam"/></div>
      <div><label className="form-label">Current Use</label>
      ...
      <div className="col-span-2"><label className="form-label">GPS Coordinates</label>
        <input className="form-input" value={form.gps} onChange={e=>f('gps',e.target.value)} placeholder="e.g. -0.2833, 36.0667"/></div>
      <div className="col-span-2"><label className="form-label">Notes</label>
        <textarea className="form-input resize-y min-h-[60px]" value={form.notes} onChange={e=>f('notes',e.target.value)}/></div>
...
function CropPlanForm({ plots, onClose }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2"><label className="form-label">Plot<span className="text-red-500">*</span></label>
      ...
      <div><label className="form-label">Crop Type<span className="text-red-500">*</span></label>
        <select className="form-input" value={form.cropType} onChange={e=>f('cropType',e.target.value)}>
          <option value="">Select…</option>
          {['Napier Grass','Maize','Lucerne','Rhodes Grass','Kikuyu Grass','Sweet Potato Vines','Sorghum','Sugar Cane','Other'].map(c=><option key={c}>{c}</option>)}
        </select></div>
      <div><label className="form-label">Variety</label>
        <input className="form-input" value={form.variety} onChange={e=>f('variety',e.target.value)} placeholder="e.g. KH500"/></div>
      <div><label className="form-label">Planting Date</label>
        <input className="form-input" type="date" value={form.plantingDate} onChange={e=>f('plantingDate',e.target.value)}/></div>
      <div><label className="form-label">Expected Harvest</label>
        <input className="form-input" type="date" value={form.expectedHarvest} onChange={e=>f('expectedHarvest',e.target.value)}/></div>
      <div><label className="form-label">Seeds Used (kg)</label>
        <input className="form-input" type="number" value={form.seedsKg} onChange={e=>f('seedsKg',e.target.value)}/></div>
      <div><label className="form-label">Fertilizer Applied</label>
        <input className="form-input" value={form.fertilizer} onChange={e=>f('fertilizer',e.target.value)} placeholder="e.g. CAN 50kg"/></div>
      <div className="col-span-2"><label className="form-label">Notes</label>
        <textarea className="form-input resize-y min-h-[50px]" value={form.notes} onChange={e=>f('notes',e.target.value)}/></div>
      <div className="col-span-2 flex justify-end gap-3 pt-2 border-t border-[#e8e0d0]">
        <button className="btn btn-secondary" onClick={onClose}>Cancel</button>
...
export default function Crops() {
  ...
  return (
    <div className="page-content">
      ...
      <div className="card">
        {tab==='plots'    && <DataTable columns={plotCols}    rows={plots||[]}    emptyText="No plots registered. Click 'Add Plot' to add your first field."/>}
